...
class RCModel(object):
    ...
    def _decode(self):
        ...
        with tf.variable_scope('same_question_concat'):
            ...
            # )
            concat_passage_encodes = self.fuse_p_encodes        # (b*5, nums, hidden)
            # no_dup_question_encodes = tf.reshape(
            #     self.match_q_encodes,
            #     [batch_size, -1, tf.shape(self.match_q_encodes)[1], 2 * self.hidden_size]
            # )[0:, 0, 0:, 0:]
            no_dup_question_encodes = self.match_q_encodes      # (b*5, nums, hidden)
    ...
    def evaluate(self, eval_batches, result_dir=None, result_prefix=None, save_full_info=False):
        ...
        for b_itx, batch in enumerate(eval_batches):
            ...
            count = 0
            for sample in batch['raw_data']:     # batch['raw_data']是batch个，start_probs 维度是40  b*5 ndarray
                # 对sample处理一下，增加一个循环，便利里面所有的问题
                tmp = sample.copy()
                for i in range(len(tmp['question'])):
                    tmp['question'] = sample['question'][i]
                    tmp['question_token_ids'] = sample['question_token_ids'][i]
                    tmp['questions_token'] = sample['questions_token'][i]
                    best_answer = self.find_best_answer(tmp, start_probs[count], end_probs[count], padded_p_len)
                    count += 1
                    if save_full_info:
                        sample['pred_answers'] = [best_answer]
                        pred_answers.append(sample)
                    else:
                        pred_answers.append({'id': sample['paragraphs'][0]['qas'][i]['id'],
                                             'casename': sample['casename'],
                                             'answers': [best_answer],
                                             'entity_answers': [[]],
                                             'yesno_answers': []})
                    temp = []
                    temp.append(sample['answer'][i]['text'])
                    if 'answer' in sample:
                        ref_answers.append({'id': sample['paragraphs'][0]['qas'][i]['id'],
                                             'casename': sample['casename'],
                                             'answers': temp,
                                             'entity_answers': [[]],
                                             'yesno_answers': []})
        # pred_answers and ref_answers['answers']中有多个答案，所以是一个list
    ...
    def find_best_answer(self, sample, start_prob, end_prob, padded_p_len):
        ...
        # for p_idx, passage in enumerate(sample['question']):
        #     if p_idx >= self.max_p_num:
        #         continue
        passage_len = min(self.max_p_len, len(sample['question']))
        answer_span, score = self.find_best_answer_for_passage(start_prob, end_prob, passage_len)
        if score > best_score:
            best_score = score
            # best_p_idx = p_idx
        ...
        if best_span is None:
            ...
            best_answer = ''.join(
                sample['context'][best_span[0]: best_span[1] + 1])
        return best_answer
